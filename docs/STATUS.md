...
| **rag-day-32** | **Cumulative re-eval (28+29+30 stack)** (concept 32). Расширили `src/eval/runner.run_eval` двумя параметрами (`foundational_matcher`, `expand_definitional`), зеркалит логику `RAGService.query()`. Новый `scripts/eval_rag_day_32.py` — 2-cell runner (A pre-28 baseline / B post-30 stack), отчёт `docs/EVAL_RAG_DAY_32.md`. **Headline numbers (synthetic golden v0.0_extended, n=100):** `ref_hit@1` 0.190 → 0.260 (+7.0 pp), `ref_hit@5` 0.450 → 0.480 (+3.0 pp), `ref_hit@10` 0.540 → 0.590 (+5.0 pp), `ref_hit@20` 0.650 → 0.690 (+4.0 pp), MRR 0.307 → 0.360 (+0.053). **Все метрики выше нуля.** Самый сильный сигнал — `@1` jump +7pp (foundational boost реально поднимает канонические сутры на #1: qa_001/qa_040/qa_049). 9 fixed / 6 regressed at top-5. Regressions диагностически intresting: qa_062 «right effort» boost'нула `balanced effort → an6.55` (curation-bug в YAML, концепты different); 3 simile-кейса — encoder noise; qa_029 — RU glossary noise. **Decision call:** MARGINAL формально (0.480 < 0.50 threshold), но `@1`+7pp + MRR+0.053 — clear quality lift на user-visible metric. Кандидат на v0.2.0 cut. Authoritative-ness: RELATIVE only (synthetic, B-001 buddhologist still open). ~2 min wallclock на free 1080 Ti. | ✅ Done | (this branch) |
